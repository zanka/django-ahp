...
def development():
    env.env = 'development'
    env.settings = 'ahp.settings.development'


def staging():
    env.env = 'staging'
    env.settings = 'ahp.settings.staging'
    env.remote = ''
    env.heroku_app = ''
...
def production():
    env.env = 'production'
    env.settings = 'ahp.settings.production'
    env.remote = ''
    env.heroku_app = ''
...
def brunchwatch(app_name='core'):
    local('cd ahp/%s/static/ && brunch w' % app_name)


def brunchbuild(app_name='core'):
    with settings(warn_only=True):
        local('rm -r ahp/%s/static/public/' % app_name)
    local('cd ahp/%s/static/ && brunch b -m' % app_name)
